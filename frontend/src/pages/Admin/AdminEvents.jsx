...
const AdminEvents = () => {
  ...
  try {
    ...
    // ✅ CORRECT EXTRACTION: res is already the response body { success, data: { campaigns } }
  ...
  try {
    const payload = {
      ...
      status: 'Active',   // ← must be exactly "Active" (capital A)
    };

    // Optional: Add frontend validation to catch short descriptions early
  ...
  setSaving(true);
  try {
    await campaignEventsApi.createEvent(selectedCampaignId, {
            ...
            {items.map(item => (
              <div key={item._id} className="flex flex-col md:flex-row items-start md:items-center justify-between px-6 py-5 hover:bg-stone-50 transition-colors gap-4">
                ...
                <div className="flex flex-wrap gap-2 flex-shrink-0">
                   <button 
                     onClick={() => openEventModal(item._id)} 
                     className="text-xs bg-green-50 text-green-700 hover:bg-green-100 font-semibold py-2 px-3 rounded-lg border border-green-200 transition-colors flex items-center gap-1"
                   >
                     <span>+</span> Add Event
                   </button>
                   <button 
                     onClick={() => navigate('/admin/analytics')} 
                     className="text-xs bg-white text-stone-700 hover:bg-stone-100 font-semibold py-2 px-3 rounded-lg border border-stone-200 transition-colors"
                   >
                     📊 Analytics
                   </button>
                   <button 
                     onClick={() => navigate('/admin/attendance')} 
                     className="text-xs bg-violet-600 text-white hover:bg-violet-700 font-semibold py-2 px-3 rounded-lg transition-colors"
                   >
                     ✅ Attendance
                   </button>
                </div>
      ...
      <form onSubmit={handleCreateEvent} className="px-6 py-6 space-y-4">
        ...
        <div>
          ...
          <select 
            ...
            className="w-full border border-stone-200 rounded-lg px-4 py-2 focus:ring-2 focus:ring-green-500 outline-none"
          >
        ...
        {/* NOTE: description is removed per validation error */}
